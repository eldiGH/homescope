...
│       └── reading.rs      # SensorReading (serde camelCase, human units) — decoded-reading shape (future API output; currently unused)
...
- 🔶 **Packet redesign + crypto block** (settled 2026-07-16, see `NOTES-packet-tv-aead.md`; owner's order): ① ✅ DeviceAddr refactor → ② 🔶 TV measurement encoding — ✅ `common` registry + sensor encode + receiver forwarding (2026-07-25); ✅ **gateway migration** (2026-07-26: `frame::parse` decoder loop + `ObservationEnvelope` MQTT publish); ⏳ **API TV decode** (base64 → `SensorPacket::parse` → `Measurements` walk into the existing wide columns — **no DB migration needed now**; metric columns go nullable only in the same migration that adds the first optional metric, i.e. the BMP581 node; until then the ingest needs an explicit warn+drop posture for subset packets) → ③ ✅ seq persistence on the sensor (flash checkpoint w/ jump-ahead, `seq_counter.rs`; AEAD-nonce prerequisite done) → ④ ⏳ ChaCha20-Poly1305 AEAD (decrypt-in-API reaffirmed; keyless gateways; integrations later get plaintext via API republish of verified readings, not via gateway decryption).
