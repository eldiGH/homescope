...
│       ├── frame.rs        # content-agnostic framing: Encode trait + encode/parse — [magic HS][len u16][payload][crc], cap 1024, streaming 3-outcome parse
...
- ⚠️ **Gateway not yet migrated**: `gateway/src/decoder.rs` still expects v0.4 25-byte frames. The `common` side is ready (`frame::parse` incomplete/ok/corrupt + `SensorObservation::parse`); the `tokio_util::codec::Decoder` becomes a loop over `frame::parse` (Incomplete → Ok(None); Ok → advance consumed; Corrupt → advance 1), then the MQTT publish switches to the opaque envelope.
