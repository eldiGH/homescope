...
|---|---|---|---|
| **Indoor room node** | many | SHT45 (T/H); **one designated node also carries a BMP581 (P)** | Per-room temperature & humidity — the only values that genuinely vary room-to-room. ±0.1 °C / ±1 % RH, no heater, ~80 nA idle. The single barometer rides on one indoor node (see below). |
...
- **Topology (XIAO dev boards)**: AA pair → directly to XIAO `3V3` pin (bypasses on-board LDO; nRF52840 operates 1.7-3.6 V). Caveat: a fresh L91 pair is ~3.5-3.6 V open-circuit — right at the absolute-max edge; acceptable on the bench, not the final answer.
- **Topology (custom PCB — the proper fix)**: AA pair → **VDDH** (the nRF52840's high-voltage input, 2.5-5.5 V, internal REG0 buck) with `REGOUT0 = 3.0 V` feeding VDD and the sensors. Eliminates the fresh-pair absolute-max edge entirely; the 2.5 V VDDH floor ≈ 1.25 V/cell, past >95 % of Li-FeS₂ capacity (flat discharge curve, then cliff). The MDBT50Q module exposes VDDH.
- **Required passives**:
