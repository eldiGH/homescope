...
**Uncommitted scratch plans**: `NOTES-*.md` files at the repo root (untracked by design) hold the owner's worked-out plans for upcoming backend/deploy tasks — graceful shutdown, ingest DB-error handling / manual acks, site+room topology, mosquitto ACLs, udev device activation, USB-CDC tightening, and the packet TLV + seq-persistence + AEAD block (`NOTES-packet-tlv-aead.md`). When one of those topics comes up, read the matching NOTES file first; the decisions in them are settled.
...
│       ├── packet.rs       # SensorPacket (repr(C, packed)) — over-the-air payload (10 B; TLV redesign planned)
│       ├── observation.rs  # SensorObservation = device_addr + packet + receiver RSSI/age (21 B)
│       ├── frame.rs        # Frame: magic + payload + CRC-16/IBM-SDLC (25 B)
...
- 🔶 **Common crate** (DeviceAddr refactor in flight, 2026-07-16): `SensorPacket` (air, 10 B — identity no longer in the payload), `SensorObservation` (receiver→gateway, 21 B: `device_addr` + packet + RSSI/age), `SensorReading` (app/JSON), `DeviceAddr` ([u8; 6] LSB-first from AdvA/FICR `DEVICEADDR`, 12-hex-uppercase rendering; replaces `HardwareId`), `Frame` (magic + payload + CRC-16/IBM-SDLC) — see docs/protocol.md v0.4 (**25-byte frames**). Features: `wire` (bytemuck/crc), `serde`. **Next: TLV measurement encoding** (protocol v0.5, variable-length frames) — see `NOTES-packet-tlv-aead.md` + docs/architecture.md "Data model".
- ✅ **Gateway**: a pure bridge now — serial decode (`tokio_util` `Decoder` in `decoder.rs`), `received_at = now − age_ms`, JSON publish (QoS 1) to `homescope/sensors/<device-addr>/reading`. Env-configured (`MQTT_HOST`, `MQTT_PORT`, `RECEIVER_PATH` default `/dev/homescope-receiver`) via `host-util`. The old range-survey page (port 3000) was retired to the `reliability-benchmark` branch — check it out to rerun a survey.
...
- ⏳ **Packet redesign + crypto block** (settled 2026-07-16, see `NOTES-packet-tlv-aead.md`; owner's order): ① finish DeviceAddr refactor → ② TLV measurement encoding (ID registry in `common`; receiver/gateway go semantics-blind; MQTT envelope = JSON metadata + base64 blob; nullable metric columns, DB stays wide — "extensible, not generic") → ③ seq persistence on the sensor (retained RAM + flash checkpoint, AEAD-nonce prerequisite) → ④ ChaCha20-Poly1305 AEAD (decrypt-in-API reaffirmed; keyless gateways; integrations later get plaintext via API republish of verified readings, not via gateway decryption).
...
- **Payload**: `ManufacturerSpecificData` with company ID `0xFFFF` (testing) carrying a `#[repr(C, packed)]` `SensorPacket` struct — direct binary, no serialization framework; extended adv gives 254 B headroom (the planned AEAD tag never fit legacy's 31 B). Planned: **TLV measurement encoding** — `[seq][id][data]…` with a measurement-ID registry in `common` (ID = semantics + repr + scale; ID implies length); device identity = AdvA, not a payload field.
- **Security (planned)**: ChaCha20-Poly1305 AEAD with per-device keys (registry = `devices` table, decrypt in the API — gateways keyless); TLV section encrypted, `device_addr`+`seq` as AAD; nonce derived from the **persisted** seq counter (retained RAM + flash checkpoint on the sensor); the API's per-device seq monotonicity check = replay protection + multi-receiver dedup + MQTT-redelivery idempotency. Not implemented yet — see `NOTES-packet-tlv-aead.md`.

## USB-CDC wire protocol (receiver → gateway)

See [docs/protocol.md](docs/protocol.md) for the full spec (v0.4, in flight with the DeviceAddr refactor). Quick summary:

- 25-byte frame: 2-byte magic `HS` + 21-byte `SensorObservation` (`DeviceAddr` from AdvA + air-packet fields + receiver-side `rssi`/`age_ms`; no pressure field — that arrives with the TLV encoding) + 2-byte CRC-16/IBM-SDLC over the payload (little-endian on the wire).
- Planned v0.5: TLV measurement encoding ⇒ variable-length frames (payload-length byte after the magic); receiver/gateway treat the packet bytes as opaque.
